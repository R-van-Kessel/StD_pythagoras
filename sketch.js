...
function formatNum(n) {
  let str = parseFloat(n.toFixed(2)).toFixed(2);
  if (str.endsWith('.00')) {
    return str.slice(0, -3);
  }
  // Verwijder trailing nul bij één decimaal: "6.10" → "6,1"
  if (str.endsWith('0')) {
    str = str.slice(0, -1);
  }
  return str.replace('.', ',');
...
function makeRandomCube() {
  ...
  // 40% kans op eindpunt op de rib ipv de hoek
  let useRibEnd = Math.random() < 0.4;
  let ribFraction = useRibEnd ? round1(randInt(2, 8) / 10) : 1; // 0.2 t/m 0.8
  let ribAbove = round1((1 - ribFraction) * H);

  // Diagonaal naar hoek of naar punt op rib
  let d;
  if (useRibEnd) {
    // Pythagoras in de ruimte tot aan het ribpunt
    // Horizontale diagonaal bodem: √(W²+D²), dan omhoog met H*ribFraction
    let hDiag = Math.sqrt(W*W + D*D);
    d = round1(Math.sqrt(hDiag*hDiag + (H * ribFraction)*(H * ribFraction)));
  } else {
    d = round1(Math.sqrt(W*W + H*H + D*D));
  }
  ...
  return {
    ...
    cubeH: H,
    cubeD: D,
    cubeRibEnd: useRibEnd,
    cubeRibFraction: ribFraction,
    isCube: true,
...
function makeRandomCube() {
  ...
  let useRibEnd = Math.random() < 0.4;
  let ribFraction = useRibEnd ? round1(randInt(3, 8) / 10) : 1;

  let d;
  if (useRibEnd) {
    // A=(0,0,0) naar punt op achterste linkerrib=(0,D,H*ribFraction)
    // Diagonaal = √(W² + D² + (H*ribFraction)²)
    // Maar A naar (0,D,H*ribFraction) gaat niet door het lichaam...
    // Correct: A=(0,0,0) naar (W, D, H*ribFraction) — door de balk heen
    d = round1(Math.sqrt(W*W + D*D + (H * ribFraction)*(H * ribFraction)));
  } else {
    d = round1(Math.sqrt(W*W + H*H + D*D));
  }
  ...
  for (let row = 1; row < 3; row++) {
    for (let col = 0; col < COLS; col++) {
      ...
      blocks.push({
        ...
        cubeRibEnd:      q.cubeRibEnd      || false,
        cubeRibFraction: q.cubeRibFraction || 1,
        answer: q.answer,
        isQuestion: true,
        isCorrect: null,
        isHovered: false,
...
function drawCubeQuestion(block) {
  ...
  // Eindpunt rode lijn
  let useRibEnd = block.cubeRibEnd || false;
  let ribFraction = block.cubeRibFraction || 1;
  let diagEnd;

  if (useRibEnd) {
    // Eindpunt op RECHTERSTE ACHTERSTE rib F→G
    // ribFraction 0 = bij F (onder), 1 = bij G (boven)
    // Lijn loopt van A (voorste onderlinks) door de balk naar dit punt
    diagEnd = {
      x: F.x + (G.x - F.x) * ribFraction,
      y: F.y + (G.y - F.y) * ribFraction
    };
  } else {
    diagEnd = G;
  }
  ...
  quad(A.x,A.y, B.x,B.y, F.x,F.y, E.x,E.y);
  fill(173, 216, 255, 220);
  quad(A.x,A.y, B.x,B.y, C.x,C.y, Dpt.x,Dpt.y);
  fill(140, 190, 240, 220);
  quad(B.x,B.y, F.x,F.y, G.x,G.y, C.x,C.y);
  fill(220, 240, 255, 220);
  quad(Dpt.x,Dpt.y, C.x,C.y, G.x,G.y, Hpt.x,Hpt.y);
  fill(160, 200, 245, 160);
  quad(A.x,A.y, Dpt.x,Dpt.y, Hpt.x,Hpt.y, E.x,E.y);
  ...
  // ── Rode diagonaal achter voorste rib ──
  stroke(220, 0, 0);
  strokeWeight(3);
  line(A.x, A.y, diagEnd.x, diagEnd.y);

  // ── Voorste ribben over rode lijn heen ──
  stroke(30, 60, 120);
  strokeWeight(1.5);
  noFill();
  ...
  // ── Labels exact op midden van rib ──
  noStroke();
  textSize(10);
  textStyle(BOLD);
  textAlign(CENTER, CENTER);
  ...
  // Breedte W — onder midden rib A→B
  text(formatNum(block.cubeW), (A.x+B.x)/2, (A.y+B.y)/2 + 12);
  // Hoogte H — links midden rib A→Dpt
  text(formatNum(block.cubeH), (A.x+Dpt.x)/2 - 15, (A.y+Dpt.y)/2 -15);
  // Diepte D — links midden rib A→E
  text(formatNum(block.cubeD), (A.x+E.x)/2 - 10, (A.y+E.y)/2 + 8);

  if (useRibEnd) {
    // Onderste stukje rib (F → diagEnd)  ← nieuw
    let ribBelow = round1(ribFraction * block.cubeH);
    text(formatNum(ribBelow), F.x + 8, (F.y + diagEnd.y) / 2);
  }
  // Vraagteken bij rode lijn
  fill(220, 0, 0);
  text('?', (A.x + diagEnd.x)/2 - 8, (A.y + diagEnd.y)/2 - 8);
...
function checkAnswers() {
    ...
    for (let questionBlock of blocks) {
        if (questionBlock.isQuestion) {
            ...
            if (answerBlock) {
                // Zoek de verwachte antwoordtekst voor dit vraagblok
                let expectedText = '';
                for (let block of blocks) {
                    if (!block.isQuestion && block.answer === questionBlock.answer) {
                        expectedText = block.text;
                        break;
                    }
                }
                
                // Correct als de tekst overeenkomt (werkt ook bij duplicaten)
                let isCorrect = answerBlock.text === expectedText;
                
                questionBlock.isCorrect = isCorrect;
                answerBlock.isCorrect = isCorrect;
